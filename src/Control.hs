...
control s ev = case ev of 
  AppEvent Tick                   -> next s =<< liftIO (play O s)
  T.VtyEvent (V.EvKey V.KEnter _) -> next s =<< liftIO (play X s)
...
play xo s
  ...
  | otherwise      = return $ Cont (psBoard s)
...
-------------------------------------------------------------------------------
next :: PlayState -> Result Board -> EventM n (Next PlayState)
-------------------------------------------------------------------------------
next s Retry     = continue s
next s (Cont b') = continue (s { psBoard  = b', psTurn = flipXO (psTurn s)})
next s (Win xo)  = halt     (s { psResult = Win xo})
next s Draw      = halt     (s { psResult = Draw})
